...
var canvas, ctx, time, camera, cssFilesArray = [];
...
function init() {
  ...
  map1.setMapObject(600,300,'Put CSS Files in Here','dropOffFolder');
  ...
  if (!isEmpty(map.mapObjects)) {
    ...
    for(var prop in objs) {
      if(cursorRect.within(objs[prop].rect) || cursorRect.overlaps(objs[prop].rect)) {
        // Cursor is within an object, return the objects name and type
        return {
          name: objs[prop].name,
          type: objs[prop].type 
        };
...
player.dropOffFile = function(obj, fileArray) {
  // Drop the file off and set to true if files array
  console.log('Dropped off ' + this.file.name + ' in ' + obj.name + ' folder');
  
  // Record that file was successfully dropped off in array
  fileArray[this.file.name] = true;
...
player.interactHandler = function(xView, yView, map, fileArray) {
  ...
  if(inObject.type === 'file' && !this.hasFile) {
    ...
    this.dropOffFile(inObject, fileArray);
...
canvas.addEventListener('dbltap', function(evt){
  player.interactHandler(gCamera.xView, gCamera.yView, map1, cssFilesArray);
}, false);
