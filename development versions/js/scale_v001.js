...
var canvas, ctx, time, scale, camera;
...
function init() {
  ...
  map1 = new Map(1366*scale, 768*scale, mapImages.desktop);
  
  map1.rectangle = new Rectangle(0,0,1366*scale,768*scale);
  
  gCamera = new Camera(250*scale, 200*scale, canvas.width, canvas.height, 1366*scale, 768*scale);
  gCamera.follow(player.cursor, 100*scale, 100*scale);
...
player.drawCursor = function(ctx) {
  ...
  for(i = 0; i < cursorImg.x.length; i++) {
    ctx.lineTo(x+(cursorImg.x[i] *scale),y+(cursorImg.y[i]*scale));
  }
...
Map.prototype.draw = function(ctx, xView, yView) {
  ...
  ctx.drawImage(this.image,sx/0.5,sy/0.5,sWidth/0.5,sHeight/0.5,dx,dy,300,225);
};
