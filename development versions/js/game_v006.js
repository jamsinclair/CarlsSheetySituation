...
// Ponder: gameImages class with draw functions?
...
function init() {
  ...
  map1.setMapObject(1200,100,'test\.css','file');
  map1.setMapObject(100,100,'layoutcssab','file');
  map1.setMapObject(600,300,'Put CSS Files in Here','folder');
...
player.inObject = function(xView, yView, map) {
  var cursorRect = new Rectangle(this.cursor.x+xView, this.cursor.y+yView,1,1);
  
  // Does map have any objects?
  if (!isEmpty(map.mapObjects)) {
    var objs = map.mapObjects;
    // Loop through it's objects
    for(var prop in objs) {
      if(cursorRect.within(objs[prop].rect) || cursorRect.overlaps(objs[prop].rect)) {
        // Cursor is within object
        
        if (objs[prop].type === 'file' && !this.hasFile) {
          // Pick up file and delete it from map objects
          console.log('Picked up file ' + objs[prop].name + ' at ' + objs[prop].x + ', ' + objs[prop].y);
          this.cursor.type = 'drag';
          this.hasFile = true;
          this.file = objs[prop];
          delete objs[prop];
          break;
          return true;
        } else if (objs[prop].type === 'dropFolder' && this.hasFile) {
          return true;
        }
      }  
    }
...
player.placeObject = function(xView, yView, map) {
  var x,y;
  ...
  map.setMapObject(x,y,this.file.name, this.file.type);
...
player.interactHandler = function(xView, yView, map) {
  
  if(!player.hasFile) {
    // Player is not carrying a file are they in an object?
    player.inObject(xView, yView, map);
  } else if(player.hasFile) {
    if (!player.inObject(xView, yView, map)) {
      // Cursor not currently in an object, place object on map
      player.placeObject(xView, yView, map);
    }
  }
