...
function init() {
  ...
  Game.setMap(1366, 768, mapImages['desktop'], 'desktop');
  Game.setMap(1366, 768, mapImages['forgetFolder'], 'forgetFolder');
  if(isEmpty(localStorage.getItem('css1')))
    Game.maps['desktop'].setMapObject(1200,100,'blue.css','file', 'css1');
  if(isEmpty(localStorage.getItem('css2')))
    Game.maps['desktop'].setMapObject(100,100,'red.css','file', 'css2');
  Game.maps['desktop'].setMapObject(500,320,'Put CSS Files in Here','dropOffFolder');
  Game.maps['desktop'].setMapObject(1050,450,'I forget what\'s in here','folder', 'forgetFolder');
  Game.maps['desktop'].setMapObject(1195,740,'clock','clock');
  Game.maps['forgetFolder'].setMapObject(1195,740,'clock','clock');
  Game.maps['forgetFolder'].setMapObject(45,170,'backBtn','link','desktop',139,310);
  Game.maps['forgetFolder'].setMapObject(1270,10,'exitBtn','link','desktop',70,70);
  
  Game.currentMap = Game.maps['desktop'].mapName;
...
Game.changeMap = function(mapName) {
  ...
  this.player.cursor.x = canvas.width/2;
  this.player.cursor.y = canvas.height/2;
  // TODO: change this to game.camera when added
  // Centre viewport to center of map
  gCamera.xView = this.maps[mapName].width/4;
  gCamera.yView = this.maps[mapName].height/4;
  // update current map
...
function draw(ctx) {
  ...
  Game.maps[Game.currentMap].draw(ctx, gCamera.xView, gCamera.yView, gCamera.viewportRect);
...
canvas.addEventListener('dbltap', function(evt){
  Game.interactHandler(gCamera.xView, gCamera.yView);
}, false);
